...
export function createCohortSubprocessWorker({
  ...
  return async (contract, context = {}) => {
    ...
    try {
      ...
      throw new Error(`cohort worker packet is missing or malformed: ${error.message}`);
    }
    ...
    if (envelope?.schemaVersion !== 'cohort-call-packet-v1'
        ...
        || !envelope.packet
        || stableJson(JSON.parse(readFileSync(contractPath, 'utf8')))
          !== stableJson(persistedContract)) {
      throw new Error('cohort worker wrapper returned an unbound packet');
    }
