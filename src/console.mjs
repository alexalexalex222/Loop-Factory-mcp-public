...
function buildVNextCampaignSeriesSnapshot(state) {
  const completed = Array.isArray(state.completedWaves) ? state.completedWaves : [];
  const queued = Array.isArray(state.queue) ? state.queue : [];
  const usage = completed.flatMap((wave) => (
    Array.isArray(wave.budgetUsage) ? wave.budgetUsage : []
  ));
  const callsObserved = usage.reduce((sum, row) => (
    sum + (integer(row?.calls) ?? 0)
  ), 0);
  const inputTokens = usage.reduce((sum, row) => (
    sum + (integer(row?.inputTokens) ?? 0)
  ), 0);
  const outputTokens = usage.reduce((sum, row) => (
    sum + (integer(row?.outputTokens) ?? 0)
  ), 0);
  const generations = completed.map((wave) => ({
    generation: integer(wave.position) ?? 0,
    status: text(wave.status, 50),
    causalPass: wave.outcome?.causalPass === true,
    calibrationQualified: null,
    mutationPlanId: id(wave.configId),
    childRunId: null,
    evidenceSha256: hash(wave.evidenceSha256),
    calls: (wave.budgetUsage || []).reduce((sum, row) => (
      sum + (integer(row?.calls) ?? 0)
    ), 0)
  }));
  const decisions = completed.map((wave) => {
    const activationEligible = wave.outcome?.activationEligible === true;
    const causalPass = wave.outcome?.causalPass === true;
    return {
      id: id(wave.configId) || id(wave.waveId),
      generation: integer(wave.position) ?? 0,
      status: activationEligible
        ? 'AUTO_ADMITTED'
        : causalPass && wave.status === 'VERIFIED'
          ? 'EVIDENCE_BANKED'
          : 'REJECTED',
      authority: activationEligible
        ? 'independent-vnext-verifier'
        : causalPass && wave.status === 'VERIFIED'
          ? 'development-partition-firewall'
          : 'measured-failure-or-invalid-evidence',
      reason: activationEligible
        ? 'Validation evidence cleared the routing-only activation gate.'
        : causalPass && wave.status === 'VERIFIED'
          ? 'Causal development evidence was banked; routing remained disabled.'
          : wave.status === 'AMBIGUOUS_DISPATCH'
            ? 'Dispatch outcome was ambiguous and was never retried.'
            : wave.status === 'INVALID'
              ? 'Wave evidence failed closed.'
              : 'The candidate did not clear causal admission.',
      evidenceSha256: hash(wave.evidenceSha256)
    };
  });
  const verified = completed.filter((wave) => wave.status === 'VERIFIED');
  const current = state.currentWave?.descriptor || queued[0] || null;
  const canStop = state.operatorStop !== true && state.status !== 'BLOCKED';
  return recursiveBaseSnapshot(state, {
    enabled: true,
    mode: 'vnext-campaign',
    experimentValid: completed.length > 0
      ? verified.length === completed.length
      : null,
    causalPass: completed.length > 0
      ? verified.some((wave) => wave.outcome?.causalPass === true)
      : null,
    calibrationQualified: null,
    promotionEnabled: false,
    callsObserved,
    callsMaximum: integer(state.totalAuthorizedCalls) ?? 0,
    taskCount: (state.taskIdentities || []).filter((value) => (
      typeof value === 'string' && value.startsWith('task:')
    )).length,
    artifactCount: completed.reduce((sum, wave) => (
      sum + (wave.evidenceSha256 ? 1 : 0)
        + (wave.budgetUsageSha256 ? 1 : 0)
        + (Array.isArray(wave.budgetUsage) ? wave.budgetUsage.length : 0)
    ), 0),
    current: {
      generation: integer(current?.position) ?? completed.length,
      phase: text(state.currentWave?.phase || state.status, 50),
      childRunId: null,
      nextAction: state.operatorStop
        ? 'Stopped by operator; no later wave may start.'
        : state.status === 'BLOCKED'
          ? 'Campaign is blocked pending evidence repair.'
          : state.currentWave
            ? 'Complete or cold-recover the persisted wave dispatch.'
            : queued.length
              ? 'Dispatch the next sealed wave at the durable boundary.'
              : 'Wait without inference for another sealed wave.'
    },
    stages: completed.map((wave) => ({
      id: id(wave.waveId),
      label: `Wave ${(integer(wave.position) ?? 0) + 1}`,
      status: text(wave.status, 50),
      calls: (wave.budgetUsage || []).reduce((sum, row) => (
        sum + (integer(row?.calls) ?? 0)
      ), 0),
      maximumCalls: integer(wave.maximumCalls),
      candidateMean: null,
      lower95: null,
      shamMean: null,
      noiseUpper95: null,
    ...
    generations,
    decisions,
    memory: verified.map((wave) => ({
      id: id(wave.waveId),
      lifecycle: wave.outcome?.activationEligible === true ? 'active' : 'replicated',
      priority: wave.outcome?.causalPass === true ? 1 : 0,
      artifactSha256: hash(wave.evidenceSha256),
      semanticSha256: hash(wave.budgetUsageSha256)
    })),
    context: null,
    tokenUsage: {
    ...
    queue: {
      ...
      currentWaveId: id(state.currentWave?.descriptor?.waveId),
      completed: completed.length
    },
    operator: {
      canStop,
      stopRequested: state.operatorStop === true,
      stopEndpoint: `/api/v1/runs/${encodeURIComponent(state.runId)}/stop`
    }
  });
}
...
export function buildConsoleSnapshot(state) {
  ...
  if (state && state.kind === 'vnext-campaign-series') {
    return buildVNextCampaignSeriesSnapshot(state);
  }
