...
export function buildMechanismRoutingDecision({
  ...
  if (operatorControl != null
      && validateVNextOperatorControlProjection(operatorControl).status !== 'OK') {
    return {
      status: 'REFUSED',
      code: 'INVALID_OPERATOR_CONTROL',
      message: 'Adaptive routing needs a valid immutable operator-control projection.'
    };
  }
  ...
  const policyQuarantines = policyEpoch.quarantinedFamilyIds || [];
  const operatorQuarantines = operatorControl?.quarantinedFamilyIds || [];
  const shadowOnly = operatorControl?.shadowOnlyFamilyIds || [];
  const quarantined = new Set([
    ...policyQuarantines,
    ...operatorQuarantines,
    ...(mode === 'active-canary' ? shadowOnly : [])
  ]);
  ...
  const normalizedSeed = sha256(operatorControl
    ? `${String(seed || 'adaptive-routing-v1')}:${operatorControl.projectionSha256}`
    : String(seed || 'adaptive-routing-v1'));
  ...
  const capsulePayload = {
    ...
    operatorControlSha256: operatorControl?.projectionSha256 || null,
    candidatePoolSha256,
    items: capsuleItems
  };
  ...
  return {
    ...
    filtered: {
      ...
      policyQuarantinedFamilies: validFamilies.filter((family) => (
        policyQuarantines.includes(family.familyId)
      )).length,
      operatorQuarantinedFamilies: validFamilies.filter((family) => (
        operatorQuarantines.includes(family.familyId)
      )).length,
      shadowOnlyFamilies: validFamilies.filter((family) => (
        shadowOnly.includes(family.familyId)
      )).length,
      operatorControlSha256: operatorControl?.projectionSha256 || null
    }
  };
