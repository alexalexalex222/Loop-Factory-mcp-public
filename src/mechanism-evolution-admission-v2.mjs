...
function normalizeOutcome(value) {
  ...
  if (!exactKeys(value, fields)
      || fields.slice(0, 9).some((field) => !Number.isFinite(value[field]))
      ...
      || value.adjustedBlockP < 0 || value.adjustedBlockP > 1
...
function outcomeFrom(confirmationMeasurement, calibrationAnalysis, confirmationAnalysis) {
  return {
    ...
    adjustedBlockP:
      confirmationAnalysis.summary.adjustedBlockSignTest.pOneSided,
    adjustedTaskP:
...
export function verifyReplicatedMechanismEvolution({
  ...
  try {
    ...
    const passed = confirmationAnalysis.causalPass === true
    ...
    return createMechanismEvolutionAdmissionV2({
      ...
      reasonCodes: passed ? [] : ['REPLICATED_CAUSAL_GATES_FAILED']
    });
  } catch (error) {
