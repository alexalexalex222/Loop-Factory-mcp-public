...
export function createCodexOAuthAuthorityRecord({
  ...
  if (!isAbsolute(path) || !['codex', 'codex.real'].includes(basename(path))) {
    return refused('CODEX_BINARY_INVALID', 'Codex authority requires an absolute codex or codex.real binary path.');
  }
...
export function validateCodexOAuthAuthorityRecord(record) {
  ...
  if (record.schemaVersion !== CODEX_OAUTH_AUTHORITY_SCHEMA
      ...
      || !['codex', 'codex.real'].includes(String(record.binary?.basename || ''))
