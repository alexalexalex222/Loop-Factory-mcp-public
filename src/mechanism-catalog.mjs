...
import {
  ...
  verifyVNextRecursiveImportCommit
...
function reconcileUnlocked(paths) {
  const importGate = requireVNextRecursiveImportGate(paths.home);
  if (importGate.status !== 'OK') return importGate;
  ...
  if (missingEntries.length) {
    const existing = existsSync(paths.ledgerPath)
      ? readFileSync(paths.ledgerPath, 'utf8')
      : '';
    const separator = existing && !existing.endsWith('\n') ? '\n' : '';
    const appended = `${missingEntries
      .map((entry) => canonicalAdaptiveJson(entry)).join('\n')}\n`;
    atomicWrite(paths.ledgerPath, `${existing}${separator}${appended}`);
  } else if (!existsSync(paths.ledgerPath)) {
    atomicWrite(paths.ledgerPath, '');
  }
  return ok({
    catalog,
    recordCount: records.length,
...
export function verifyVNextRecursiveCatalogImportCommit({
  homeDir,
  runId,
  partition,
  ...
  const committed = verifyVNextRecursiveImportCommit({
    homeDir,
    transactionId: vnextImportCommit?.transactionId,
    commitSha256: vnextImportCommit?.commitSha256
  });
  return committed.status === 'OK'
      && committed.pending.runId === runId
      && committed.pending.partition === partition
      && committed.pending.verifierEvidenceSha256 === verifierEvidenceSha256
    ? committed
    : refused(
        'VNEXT_RECURSIVE_IMPORT_COMMIT_REQUIRED',
        'VNext catalog persistence requires a matching committed evidence import.',
        { committed }
      );
}
...
export function persistAdaptiveRecursiveCanaryV2Result({
  ...
  if (config.vnextBinding) {
    const committed = verifyVNextRecursiveCatalogImportCommit({
      homeDir,
      runId,
      partition: config.vnextBinding.taskPartition,
      verifierEvidenceSha256: verification.evidenceSha256,
      vnextImportCommit
    });
    const lease = verifyAdaptiveRecursiveVNextLeaseReceipt(sourceStore, runId);
    const paceRequired = state.status === 'QUEUE_DRAINED';
    if (committed.status !== 'OK' || lease.status !== 'OK'
        || verification.gates?.vnextExecutionBinding !== true
        || verification.gates?.paceEvidence !== true
        || (paceRequired && (
          validatePaceState(verification.paceState).status !== 'OK'
          || validatePaceV2GateRecord(verification.paceGateRecord).status !== 'OK'
          || verification.paceGateRecord.allV2GatesPassed
            !== verification.causalPass
        ))
        || (!paceRequired && (
          verification.paceState != null
          || verification.paceGateRecord != null
        ))) {
      return refused(
        'VNEXT_RECURSIVE_IMPORT_COMMIT_REQUIRED',
        'VNext persistence requires evidence-first import commit, released lease, execution binding, and PACE replay.',
        { committed, lease, verification }
      );
    }
  }
  const records = [
...
export function persistAdaptiveRecursiveCanaryV2DevelopmentResult({
  ...
  if (config.vnextBinding) {
    ...
    if (committed.status !== 'OK') {
      return refused(
        'VNEXT_RECURSIVE_IMPORT_COMMIT_REQUIRED',
        'VNext development persistence requires a committed verifier-owned evidence import.',
        { committed }
      );
    }
  }
...
export function listAdaptiveRecords({
  ...
  const safety = validatePaths(paths);
  if (safety.status !== 'OK') return safety;
  const importGate = requireVNextRecursiveImportGate(home);
  if (importGate.status !== 'OK') return importGate;
  ...
  return ok({
    ...
    rejected: scanned.rejected,
    vnextImportGateSha256: importGate.gateSha256
  });
}
...
export function loadMechanismCatalog({ homeDir } = {}) {
  ...
  try {
    ...
    if (catalog.schemaVersion !== MECHANISM_CATALOG_SCHEMA_VERSION
        ...
        || catalog.vnextImportGateSha256 !== importGate.gateSha256
