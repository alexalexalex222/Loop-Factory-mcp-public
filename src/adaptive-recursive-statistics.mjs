...
function confidence(values) {
  ...
  return {
    ...
    method: 'paired-task-cluster-normal-approximation'
  };
}
...
export function summarizeRecursiveReplicatedStage(inputBlocks = []) {
  try {
    ...
    const candidateDeltas = taskGroups.map((task) => task.candidateVsParentMean);
    const placeboDeltas = taskGroups.map((task) => task.shamVsColdMean);
    ...
    const summary = {
      ...
      adjusted: confidence(taskAdjusted),
...
function stageGates(summary, threshold) {
  ...
  return {
    ...
    adjustedTaskSignificant:
      taskSign.wins > taskSign.losses && taskSign.pOneSided <= 0.05,
