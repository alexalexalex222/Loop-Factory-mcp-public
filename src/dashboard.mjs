...
import { escapeHtml, sha256 } from './util.mjs';
  ...
  const rows = runs.length
    ? runs.map((snapshot, index) => {
        ...
        const mode = canary
          ? 'Canary'
          : recursive
            ? (recursive.mode === 'campaign'
                ? 'Recursive campaign'
                : recursive.mode === 'vnext-campaign'
                  ? 'VNext campaign'
                  : 'Recursive canary')
            : (snapshot.realTest && snapshot.realTest.enabled ? 'Strict campaign' : 'Campaign');
        const routingAdmitted = recursive?.decisions?.some((decision) => (
          decision.status === 'AUTO_ADMITTED'
        ));
        const evidenceBanked = recursive?.decisions?.some((decision) => (
          decision.status === 'EVIDENCE_BANKED'
        ));
        ...
        const tone = !canary && pendingDecisions > 0
          ...
          : canaryTone(canary
            ...
            : routingAdmitted || evidenceBanked ? 'PASS' : snapshot.run.status);
...
function renderRecursiveDashboard(view, options = {}) {
  ...
  const stateRevisionSha256 = options.stateRevisionSha256
  ...
  const disposition = recursive.experimentValid
    ? (routingAdmitted
        ...
        : evidenceBanked
          ? 'Causal evidence banked'
          : 'Valid evidence, no admission')
    : (view.run.status === 'RUNNING' ? 'Measurement in progress' : 'Evidence not yet valid');
  const dispositionTone = recursive.experimentValid
    ? (routingAdmitted || evidenceBanked ? 'success' : 'neutral')
    : canaryTone(view.run.status);
  const generationRows = recursive.generations.length
    ? recursive.generations.map((generation) => {
        const decision = recursive.decisions.find((item) => (
          item.generation === generation.generation
        ));
        const admitted = decision?.status === 'AUTO_ADMITTED';
        const banked = decision?.status === 'EVIDENCE_BANKED';
        return `<button class="generation-row ${admitted || banked ? 'is-admitted' : ''}" type="button" data-generation="${generation.generation}">
        ...
        <span class="status-pill state-${canaryTone(admitted || banked ? 'PASS' : generation.status)}">${admitted ? 'Admitted' : banked ? 'Banked' : 'Measured'}</span>
      </button>`;
      }).join('')
  ...
  const decisionRows = recursive.decisions.length
    ? recursive.decisions.map((decision) => {
        const admitted = decision.status === 'AUTO_ADMITTED';
        const banked = decision.status === 'EVIDENCE_BANKED';
        return `<article class="decision-row state-${canaryTone(admitted || banked ? 'PASS' : 'FAIL')}">
        <div class="decision-icon">${admitted ? 'A' : banked ? 'B' : 'D'}</div>
        <div><span class="eyebrow">Generation ${decision.generation}</span><h3>${admitted ? 'Auto-admitted' : banked ? 'Evidence banked' : 'Denied routing'}</h3><p>${displayValue(decision.reason)}</p></div>
  ...
  const operatorControl = recursive.operatorControl || null;
  const releaseEvidenceSha256 = recursive.latestWave?.partition === 'validation'
    ? recursive.latestWave.evidenceSha256
    : null;
  const controlFamilyRows = operatorControl?.families?.length
    ? operatorControl.families.map((family) => {
        const canQuarantine = ['ROUTING_ELIGIBLE', 'OBSERVED', 'SHADOW_ONLY']
          .includes(family.status);
        const canRelease = family.status === 'QUARANTINED' && releaseEvidenceSha256;
        const action = canRelease
          ? `<button class="control-button" type="button" data-control-action="release-quarantine" data-target-type="family" data-target-id="${escapeHtml(family.id)}" data-target-sha="${escapeHtml(family.sha256)}" data-verifier-sha="${escapeHtml(releaseEvidenceSha256)}">Release to shadow</button>`
          : canQuarantine
            ? `<button class="control-button is-danger" type="button" data-control-action="quarantine-family" data-target-type="family" data-target-id="${escapeHtml(family.id)}" data-target-sha="${escapeHtml(family.sha256)}">Quarantine</button>`
            : '<span class="control-locked">Verifier held</span>';
        return `<div class="control-row">
          <div><strong>${escapeHtml(family.id)}</strong><small>${displayValue(family.lifecycle)} / +${displayInteger(family.positiveEvidenceCount)} / !${displayInteger(family.contradictionEvidenceCount)}</small></div>
          <span class="status-pill state-${canaryTone(family.status === 'ROUTING_ELIGIBLE' ? 'PASS' : family.status === 'QUARANTINED' || family.status === 'EVIDENCE_QUARANTINED' ? 'FAIL' : 'UNKNOWN')}">${displayValue(family.status)}</span>
          ${action}
        </div>`;
      }).join('')
    : '<div class="empty-row">No mechanism families are available.</div>';
  const controlPolicyRows = operatorControl?.policyHeads?.length
    ? operatorControl.policyHeads.map((policy) => `<div class="policy-control-row">
        <div><strong>${escapeHtml(policy.scopeId)}</strong><small class="mono">E${displayInteger(policy.epochNumber)} / ${shortHash(policy.sha256)}</small></div>
        <div class="rollback-actions">${policy.ancestors.length
          ? policy.ancestors.map((ancestor) => `<button class="control-button" type="button" data-control-action="rollback-policy" data-target-type="policy" data-target-id="${escapeHtml(policy.id)}" data-target-sha="${escapeHtml(policy.sha256)}" data-rollback-id="${escapeHtml(ancestor.id)}" data-rollback-sha="${escapeHtml(ancestor.sha256)}">Rollback to E${displayInteger(ancestor.epochNumber)}</button>`).join('')
          : '<span class="control-locked">Baseline epoch</span>'}</div>
      </div>`).join('')
    : '<div class="empty-row">No adaptive policy epoch is available.</div>';
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);min-height:100vh}button{font:inherit}.mono{font-family:"SFMono-Regular",Consolas,"Liberation Mono",monospace}.eyebrow{display:block;color:#5ed69b;font:700 10px/1.4 "SFMono-Regular",monospace;text-transform:uppercase;letter-spacing:0}.shell{min-height:100vh;display:grid;grid-template-rows:64px 1fr}.topbar{display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);align-items:center;border-bottom:1px solid var(--line2);padding:0 24px;position:sticky;top:0;background:rgba(7,9,8,.96);z-index:10}.brand{display:flex;align-items:center;gap:12px;min-width:0}.brand-mark{width:30px;height:30px;border:1px solid var(--green2);display:grid;place-items:center;color:var(--green);border-radius:5px;font-weight:900}.brand strong{font-size:14px}.brand small{display:block;color:var(--muted);font-size:11px;margin-top:2px}.run-title{text-align:center;min-width:0}.run-title strong{display:block;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.run-title small{color:var(--muted);font-size:11px}.top-actions{display:flex;justify-content:flex-end;align-items:center;gap:10px}.connection{display:flex;align-items:center;gap:7px;color:var(--muted);font-size:11px}.connection i{width:7px;height:7px;background:var(--green);border-radius:50%;box-shadow:0 0 0 3px rgba(57,217,138,.12)}.stop-button{border:1px solid #69383b;background:#211315;color:#ffb7bb;border-radius:5px;padding:8px 11px;cursor:pointer;font-weight:700}.stop-button:hover{border-color:var(--red)}.stop-button:disabled{opacity:.45;cursor:not-allowed}.workspace{display:grid;grid-template-columns:246px minmax(0,1fr) 310px;min-height:0}.rail{border-right:1px solid var(--line2);padding:22px 14px;min-width:0}.rail-head{padding:0 8px 14px}.rail-head h2,.side-panel h2{font-size:13px;margin:5px 0 0}.generation-list{display:grid;gap:4px}.generation-row{width:100%;display:grid;grid-template-columns:36px minmax(0,1fr) auto;align-items:center;text-align:left;gap:8px;border:1px solid transparent;background:transparent;color:var(--text);padding:10px 8px;border-radius:5px;cursor:pointer}.generation-row:hover,.generation-row.is-admitted{background:var(--panel);border-color:var(--line)}.generation-number{color:var(--muted);font-size:11px}.generation-copy{min-width:0}.generation-copy strong,.generation-copy small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.generation-copy strong{font-size:12px}.generation-copy small{font-size:10px;color:var(--muted);margin-top:3px}.status-pill{display:inline-flex;align-items:center;white-space:nowrap;border:1px solid var(--line);border-radius:999px;padding:4px 7px;font-size:9px;font-weight:800;text-transform:uppercase}.state-success{color:#8ff0bd;border-color:#246945;background:#10271c}.state-failure{color:#ff9da3;border-color:#6b3438;background:#251416}.state-warning{color:#ffd27c;border-color:#705525;background:#271f10}.state-neutral{color:#b8c5bf}.main{min-width:0}.run-strip{padding:24px 28px 20px;border-bottom:1px solid var(--line2)}.run-strip-top{display:flex;justify-content:space-between;gap:20px;align-items:flex-start}.run-strip h1{font-size:24px;line-height:1.15;margin:5px 0 8px;letter-spacing:0}.run-strip p{margin:0;color:var(--muted);font-size:13px;max-width:700px}.proof-score{text-align:right}.proof-score strong{font-size:30px;line-height:1}.proof-score small{display:block;color:var(--muted);font-size:10px;margin-top:5px;text-transform:uppercase}.call-progress{height:4px;background:var(--line2);margin-top:20px;overflow:hidden}.call-progress i{display:block;height:100%;background:var(--green)}.call-caption{display:flex;justify-content:space-between;color:var(--muted);font-size:10px;margin-top:7px}.tabs{height:46px;display:flex;align-items:end;gap:22px;padding:0 28px;border-bottom:1px solid var(--line2)}.tab{height:46px;border:0;border-bottom:2px solid transparent;background:transparent;color:var(--muted);cursor:pointer;padding:0;font-weight:700;font-size:12px}.tab.is-active{color:var(--text);border-color:var(--green)}.tab-panel{display:none;padding:24px 28px 40px}.tab-panel.is-active{display:block}.stage-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));border:1px solid var(--line);border-radius:6px;overflow:hidden}.stage-block{padding:20px;background:var(--panel);min-width:0}.stage-block+ .stage-block{border-left:1px solid var(--line)}.stage-block header{display:flex;justify-content:space-between;gap:12px;align-items:start}.stage-block h3{font-size:15px;margin:4px 0 0}.stage-progress{height:3px;background:var(--line2);margin:16px 0}.stage-progress i{display:block;height:100%;background:var(--blue)}.metric-strip{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;padding-bottom:16px;border-bottom:1px solid var(--line2)}.metric-strip small,.decision-authority small,.context-stat small,.token-stat small{display:block;color:var(--muted);font-size:9px;text-transform:uppercase}.metric-strip strong{display:block;font-size:13px;margin-top:4px}.arm-table{padding-top:10px}.arm-row{display:grid;grid-template-columns:95px 48px minmax(50px,1fr) 66px;align-items:center;gap:8px;min-height:29px;font-size:10px}.arm-row strong{text-align:right}.arm-row small{text-align:right;color:var(--muted)}.arm-meter{height:3px;background:var(--line2)}.arm-meter i{display:block;height:100%;background:var(--green)}.section-head{display:flex;justify-content:space-between;align-items:end;margin:26px 0 10px}.section-head h2{font-size:15px;margin:4px 0 0}.section-head span:last-child{color:var(--muted);font-size:10px}.decision-ledger,.gate-list,.memory-list,.event-list{border-top:1px solid var(--line)}.decision-row{display:grid;grid-template-columns:34px minmax(0,1fr) 190px;gap:14px;align-items:center;padding:15px 4px;border-bottom:1px solid var(--line2);background:transparent}.decision-icon{width:28px;height:28px;border:1px solid currentColor;border-radius:4px;display:grid;place-items:center;font-weight:900}.decision-row h3{font-size:13px;margin:3px 0}.decision-row p{font-size:11px;color:var(--muted);margin:0}.decision-authority{text-align:right}.decision-authority strong,.decision-authority span{display:block;font-size:10px;margin-top:3px}.gate-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:0 24px}.gate-row{display:grid;grid-template-columns:12px minmax(0,1fr) auto;gap:9px;align-items:center;min-height:38px;border-bottom:1px solid var(--line2);font-size:11px}.gate-mark{width:7px;height:7px;border-radius:50%;background:currentColor}.state-text-success{color:var(--green)}.state-text-failure{color:var(--red)}.state-text-warning{color:var(--amber)}.state-text-neutral{color:var(--muted)}.memory-row{display:grid;grid-template-columns:80px minmax(0,1fr) 110px;gap:12px;align-items:center;min-height:52px;border-bottom:1px solid var(--line2);font-size:11px}.memory-row small{display:block;color:var(--muted);margin-top:3px}.memory-state{text-transform:uppercase;font-size:9px;font-weight:800}.empty-row{color:var(--muted);font-size:11px;padding:22px 4px;border-bottom:1px solid var(--line2)}.side{border-left:1px solid var(--line2);padding:22px 18px;min-width:0}.side-panel{padding-bottom:22px;margin-bottom:22px;border-bottom:1px solid var(--line)}.context-grid,.token-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-top:15px}.context-stat strong,.token-stat strong{display:block;margin-top:4px;font-size:14px}.policy-action{display:flex;align-items:center;justify-content:space-between;margin-top:14px;padding:10px;border:1px solid var(--line);border-radius:5px}.policy-action strong{font-size:12px}.policy-action span{font-size:9px;color:var(--muted)}.event-row{padding:10px 0;border-bottom:1px solid var(--line2)}.event-row header{display:flex;justify-content:space-between;gap:8px}.event-row strong{font-size:10px}.event-row time{font-size:9px;color:var(--muted)}.event-row p{font-size:10px;color:var(--muted);margin:5px 0 0;overflow-wrap:anywhere}.hash-button{border:0;background:transparent;color:var(--muted);cursor:pointer;padding:0;font:10px/1.4 "SFMono-Regular",monospace}.hash-button:hover{color:var(--text)}.footer-boundary{font-size:10px;color:var(--muted);line-height:1.5}.footer-boundary strong{color:var(--text)}
    .control-summary{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));border:1px solid var(--line);margin-bottom:18px}.control-summary span{padding:12px;border-right:1px solid var(--line)}.control-summary span:last-child{border-right:0}.control-summary small{display:block;color:var(--muted);font-size:9px;text-transform:uppercase}.control-summary strong{display:block;margin-top:4px;font-size:12px}.control-list{border-top:1px solid var(--line)}.control-row{display:grid;grid-template-columns:minmax(0,1fr) auto 132px;gap:12px;align-items:center;min-height:58px;border-bottom:1px solid var(--line2)}.control-row strong,.control-row small,.policy-control-row strong,.policy-control-row small{display:block;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.control-row strong,.policy-control-row strong{font-size:11px}.control-row small,.policy-control-row small{margin-top:4px;color:var(--muted);font-size:9px}.control-button{min-height:34px;border:1px solid var(--line);border-radius:4px;background:var(--panel2);color:var(--text);cursor:pointer;font-size:10px;font-weight:800}.control-button:hover{border-color:var(--green)}.control-button.is-danger{border-color:#69383b;color:#ffb7bb}.control-button:disabled{opacity:.4;cursor:not-allowed}.control-locked{color:var(--muted);font-size:9px;text-align:right}.policy-control-row{display:grid;grid-template-columns:minmax(160px,.6fr) minmax(0,1fr);gap:16px;align-items:center;min-height:58px;border-bottom:1px solid var(--line2)}.rollback-actions{display:flex;justify-content:flex-end;gap:8px;overflow:auto}.rollback-actions .control-button{flex:0 0 auto;padding:0 10px}.control-boundary{margin-top:14px;color:var(--muted);font-size:10px;line-height:1.5}
...
<body>
      ...
      <main class="main">
        <section class="run-strip">
          <div class="run-strip-top">
            ...
            <div class="proof-score state-text-${dispositionTone}"><strong>${recursive.experimentValid ? (routingAdmitted ? 'ADMITTED' : evidenceBanked ? 'BANKED' : 'VALID') : displayValue(view.run.status)}</strong><small>Verifier disposition</small></div>
          </div>
        ...
        <nav class="tabs" aria-label="Run views"><button class="tab is-active" type="button" data-tab="overview">Overview</button><button class="tab" type="button" data-tab="evidence">Evidence</button><button class="tab" type="button" data-tab="memory">Memory</button>${operatorControl ? '<button class="tab" type="button" data-tab="control">Control</button>' : ''}</nav>
        ...
        ${operatorControl ? `<section class="tab-panel" data-panel="control">
          <div class="control-summary">
            <span><small>Revision</small><strong class="mono">${displayInteger(operatorControl.revision)}</strong></span>
            <span><small>Disposition</small><strong>${displayValue(operatorControl.disposition)}</strong></span>
            <span><small>Activation</small><strong>Verifier only</strong></span>
            <span><small>Promotion</small><strong>Disabled</strong></span>
          </div>
          <div class="section-head"><div><span class="eyebrow">Mechanism controls</span><h2>Routing quarantine</h2></div><span>${operatorControl.families.length} families</span></div>
          <div class="control-list">${controlFamilyRows}</div>
          <div class="section-head"><div><span class="eyebrow">Policy controls</span><h2>Bound rollback</h2></div><span>${operatorControl.policyHeads.length} scopes</span></div>
          <div class="control-list">${controlPolicyRows}</div>
          <p class="control-boundary">Every action binds this revision and immutable target bytes. Release returns to shadow only. This desk cannot activate or promote a mechanism.</p>
        </section>` : ''}
      </main>
      <aside class="side">
  ...
  <script>
    (function(){
      ...
      var stateRevisionSha256=${JSON.stringify(stateRevisionSha256)};
      var connection=document.getElementById('connection');
      ...
      document.querySelectorAll('[data-control-action]').forEach(function(button){button.addEventListener('click',function(){
        var kind=button.getAttribute('data-control-action');
        var targetId=button.getAttribute('data-target-id');
        var message=kind==='quarantine-family'?'Quarantine this mechanism from routing?':kind==='release-quarantine'?'Release this mechanism to shadow observation only?':'Rollback this policy to the selected bound epoch?';
        if(!window.confirm(message))return;
        button.disabled=true;
        var verifierSha=button.getAttribute('data-verifier-sha')||null;
        var payload={
          kind:kind,
          expectedRunRevisionSha256:stateRevisionSha256,
          target:{type:button.getAttribute('data-target-type'),id:targetId,sha256:button.getAttribute('data-target-sha')},
          expectedRevisionSha256:view.recursive.operatorControl.revisionSha256,
          rollbackTarget:kind==='rollback-policy'?{id:button.getAttribute('data-rollback-id'),sha256:button.getAttribute('data-rollback-sha')}:null,
          reasonCode:kind==='quarantine-family'?'OPERATOR_QUARANTINE':kind==='release-quarantine'?'VALIDATION_REVERIFIED':'OPERATOR_ROLLBACK',
          evidenceSha256:kind==='release-quarantine'?verifierSha:view.recursive.operatorControl.viewSha256,
          verifierEvidenceSha256:verifierSha
        };
        fetch('/api/v1/runs/'+encodeURIComponent(runId)+'/operator-control',{method:'POST',headers:{'content-type':'application/json','x-super-loop-decision-token':token},body:JSON.stringify(payload)}).then(function(response){return response.json().then(function(body){if(!response.ok)throw new Error(body.error||'operator action failed');return body;});}).then(function(){window.location.reload();}).catch(function(error){button.disabled=false;window.alert(error.message);});
      });});
      ...
      var stop=document.getElementById('stopRun');if(stop){stop.addEventListener('click',function(){if(!window.confirm('Stop this recursive run after the current durable boundary?'))return;stop.disabled=true;fetch('/api/v1/runs/'+encodeURIComponent(runId)+'/stop',{method:'POST',headers:{'content-type':'application/json','x-super-loop-decision-token':token},body:JSON.stringify({expectedRevisionSha256:stateRevisionSha256})}).then(function(response){return response.json().then(function(body){if(!response.ok)throw new Error(body.error||'stop failed');return body;});}).then(function(){stop.textContent='Stop requested';connection.querySelector('span').textContent='Stopping';}).catch(function(error){stop.disabled=false;stop.textContent='Stop run';window.alert(error.message);});});}
      function poll(){fetch('/api/v1/runs/'+encodeURIComponent(runId),{cache:'no-store'}).then(function(response){if(!response.ok)throw new Error('poll');return response.json();}).then(function(envelope){stateRevisionSha256=envelope.stateRevisionSha256;connection.querySelector('span').textContent=envelope.snapshot.recursive.operator.stopRequested?'Stop requested':'Live';connection.className='connection';}).catch(function(){connection.querySelector('span').textContent='Reconnecting';connection.className='connection state-text-warning';});}
