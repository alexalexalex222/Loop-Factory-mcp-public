...
function learningMemoryRecord(store, state, generationRecord, verification) {
  ...
  const content = canonicalJson({
    ...
    adjustedBlockP: generationRecord.adjustedBlockP,
...
export function verifyAdaptiveRecursiveCampaignRun(store, runId) {
  ...
  for (const generation of state.generations) {
    ...
    const childBlockP = child.confirmationAnalysis
      ?.summary?.adjustedBlockSignTest?.pOneSided;
    const familywisePass = child.causalPass === true
      && Number.isFinite(childBlockP)
      && childBlockP <= state.plan.statisticalAuthority.perGenerationAlpha;
    modelCalls += 1;
    ...
    const valid = mutationValid
      ...
      && generation.adjustedBlockP
        === (Number.isFinite(childBlockP) ? childBlockP : null)
...
export function runAdaptiveRecursiveCampaign(store, config, {
  ...
  while (state.nextGeneration < config.maximumGenerations) {
    ...
    const childBlockP = verification.confirmationAnalysis
      ?.summary?.adjustedBlockSignTest?.pOneSided;
    const familywisePass = verification.causalPass === true
      && Number.isFinite(childBlockP)
      && childBlockP <= state.plan.statisticalAuthority.perGenerationAlpha;
    ...
    const generationRecord = {
      ...
      adjustedBlockP: Number.isFinite(childBlockP) ? childBlockP : null,
