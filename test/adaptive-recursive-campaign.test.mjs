...
test('recursive campaign plan freezes finite exposure and no-promotion autonomy', () => {
  ...
  assert.equal(built.plan.statisticalAuthority.familywiseAdmissionReachable, true);
  assert.equal(built.plan.statisticalAuthority.routingAdmissionEnabled, true);
...
test('campaign-wide alpha keeps an ordinary child pass in development-only memory', {
  ...
  assert.equal(state.generations[0].adjustedTaskP, 0.03125);
...
test('a disjoint second generation can cite the first measured learning receipt', {
  ...
  assert.equal(state.generations[0].causalPass, false);
  assert.equal(state.generations[0].childCausalPass, true);
  assert.equal(state.generations[1].status, 'DEVELOPMENT_ADVANCE');
  assert.equal(state.generations[1].causalPass, false);
  assert.equal(state.generations[1].childCausalPass, true);
  assert.equal(state.generations[1].adjustedTaskP, 0.03125);
  assert.equal(state.generations[1].perGenerationAlpha, 0.025);
  assert.deepEqual(
