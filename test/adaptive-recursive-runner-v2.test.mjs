...
function budgetFields({ maxInputTokens = 1000, maxOutputTokens = 300 } = {}) {
  const calls = ADAPTIVE_RECURSIVE_CANARY_V2.maximumCalls;
  const policy = createResourceBudgetPolicy({
    policyId: 'recursive-v2-runner-test',
    maxCalls: calls,
    maxInputTokens: maxInputTokens * calls,
    maxOutputTokens: maxOutputTokens * calls,
    maxTotalTokens: (maxInputTokens + maxOutputTokens) * calls,
    maxUsdMicros: 0,
...
function missingPreparationBehaviorMap() {
  const core = {
    schemaVersion: 'vnext-harness-handbook-v1',
    repositoryRootSha256: sha256('missing-preparation-map'),
    ...
    behaviors: [{
      ...
      locators: [{
        ...
        sourceSha256: sha256('missing-parent-fallback'),
        locatorSha256: sha256('missing-parent-fallback')
      }],
...
function missingPreparationBinding(input) {
  const behaviorMap = missingPreparationBehaviorMap();
  const core = {
    schemaVersion: 'vnext-mechanism-execution-binding-v1',
    adapterId: 'mechanism-program-v1',
    preparationRunId: 'missing-preparation-proof',
    taskPartition: 'validation',
    taskPackSha256: sha256('missing-task-pack'),
    preparationVerifierEvidenceSha256: sha256('missing-preparation-evidence'),
    pipelineReceiptSha256: sha256('missing-pipeline-receipt'),
    candidateArtifactSha256: sha256('missing-candidate-artifact'),
    candidateVerificationSha256: sha256('missing-candidate-verification'),
    candidateContractSha256: sha256('missing-candidate-contract'),
    ...
    allowedTargetsSha256: sha256(canonicalVNextJson([{
      ...
      locatorSha256: sha256('missing-parent-fallback'),
      parentItemSha256: sha256('missing-parent-fallback')
    }])),
...
function pacePolicy() {
  const core = {
    ...
    outerAlphaAllocation: {
      allocationId: 'runner-vnext-generation-1',
      alpha: 0.05,
      familyAlpha: 0.05,
      policySha256: sha256('runner-outer-alpha-policy')
    },
...
async function preparedConfig({
  ...
  const raw = {
    ...
    ...(vnext ? budgetFields(budget) : {}),
    ...(pace ? { pacePolicy: pacePolicy() } : {})
  };
  if (bindMissingPreparation) {
    raw.vnextBinding = missingPreparationBinding(raw);
    raw.vnextBehaviorMap = missingPreparationBehaviorMap();
    raw.pacePolicy = pacePolicy();
  }
...
test('a crash after dispatch persistence leaves one unresolved budget call and never retries', {
  skip: !isDarwin
}, async () => {
  const config = await preparedConfig({ vnext: true });
  const store = createStore(mkdtempSync(join(tmpdir(), 'recursive-v2-dispatch-crash-')));
  assert.throws(() => runAdaptiveRecursiveCanaryV2WithLease(store, config, {
    runId: 'recursive-v2-dispatch-crash',
    worker: () => assert.fail('worker must not run before the crash boundary'),
    clock: () => '2026-08-04T22:30:00.000Z',
    leaseClock: () => '2026-08-04T22:30:00.000Z',
    leaseOwnerId: 'dispatch-crash-owner-a',
    leaseNonce: 'dispatch-crash-nonce-a',
    onDispatchPersisted() {
      throw new Error('simulated hard crash after dispatch persistence');
    }
  }), /simulated hard crash/);
  const state = store.load('recursive-v2-dispatch-crash');
  const budget = verifyResourceBudgetLedger(state.resourceBudgetLedger);
  assert.equal(state.dispatchIntents.length, 1);
  assert.equal(state.calls.length, 0);
  assert.equal(budget.status, 'OK');
  assert.equal(budget.unresolvedReservations.length, 1);
  assert.equal(
    state.dispatchIntents[0].budgetReservationId,
    budget.unresolvedReservations[0].reservationId
  );

  const resumed = runAdaptiveRecursiveCanaryV2WithLease(store, config, {
    runId: 'recursive-v2-dispatch-crash',
    worker: () => assert.fail('an ambiguous dispatch must never be retried'),
    clock: () => '2026-08-04T22:30:01.000Z',
    leaseClock: () => '2026-08-04T22:30:01.000Z',
    leaseOwnerId: 'dispatch-crash-owner-b',
    leaseNonce: 'dispatch-crash-nonce-b'
  });
  assert.equal(resumed.status, 'BLOCKED');
  assert.equal(resumed.code, 'AMBIGUOUS_DISPATCH_NOT_RETRIED');
  assert.equal(store.load('recursive-v2-dispatch-crash').calls.length, 0);
});

test('recursive V2 independently gates and verifies both replicated generations', {
  skip: !isDarwin
}, async () => {
  const config = await preparedConfig({ pace: true });
  ...
  assert.equal(result.verification.paceState.taskCount, 5);
  assert.equal(result.verification.paceState.wealth, 32);
  assert.equal(result.verification.paceGateRecord.allV2GatesPassed, true);
  assert.equal(result.verification.paceGateRecord.activationAuthority, false);
...
test('recursive V2 stops after calibration when replicated candidate lift is absent', {
  ...
  const result = runAdaptiveRecursiveCanaryV2WithLease(store, config, {
    runId: 'recursive-v2-calibration-reject',
    worker: workerFor(config, { candidateWorks: false }),
    clock: () => '2026-08-04T22:30:00.000Z',
    leaseClock: () => '2026-08-04T22:30:00.000Z',
    leaseOwnerId: 'runner-v2-test-owner',
    leaseNonce: 'runner-v2-test-nonce'
  });
  ...
  assert.equal(result.verification.gates.resourceBudget, true);
  assert.equal(result.verification.resourceBudgetLedger.entries.length, 120);
  assert.equal(result.verification.resourceBudgetLedger.policy.policySha256,
    config.resourceBudgetPolicy.policySha256);
  const state = store.load('recursive-v2-calibration-reject');
  assert.ok(state.evidenceArtifacts.initialResourceBudgetLedger);
  assert.ok(state.derivedArtifacts.resourceBudgetLedger);
  const leased = verifyAdaptiveRecursiveVNextLeaseReceipt(
    store,
    'recursive-v2-calibration-reject'
  );
  assert.equal(leased.status, 'OK', leased.message);
  assert.equal(leased.history.history.length, 61);

  state.resourceBudgetLedger.entries[1].inputTokens += 1;
  store.save(state);
  const tampered = verifyAdaptiveRecursiveCanaryV2Run(
    store,
    'recursive-v2-calibration-reject'
  );
  assert.equal(tampered.experimentValid, false);
  assert.equal(tampered.gates.resourceBudget, false);
  assert.equal(verifyAdaptiveRecursiveVNextLeaseReceipt(
    store,
    'recursive-v2-calibration-reject'
  ).status, 'REFUSED');
});

test('recursive V2 blocks before measurement when a VNext call breaches its reservation', {
  skip: !isDarwin
}, async () => {
  const config = await preparedConfig({
    vnext: true,
    budget: { maxInputTokens: 400, maxOutputTokens: 300 }
  });
  const store = createStore(mkdtempSync(join(tmpdir(), 'recursive-v2-budget-breach-')));
  const result = runAdaptiveRecursiveCanaryV2(store, config, {
    runId: 'recursive-v2-budget-breach',
    worker: workerFor(config),
    clock: () => '2026-08-04T22:45:00.000Z'
  });
  assert.equal(result.status, 'BLOCKED');
  assert.equal(result.code, 'RESOURCE_BUDGET_RESERVATION_BREACHED');
  const state = store.load('recursive-v2-budget-breach');
  assert.equal(state.calls.length, 0);
  assert.equal(state.resourceBudgetLedger.entries.length, 2);
  assert.equal(state.resourceBudgetLedger.entries[1].withinReservation, false);
  assert.equal(result.verification.gates.resourceBudget, false);
});

test('recursive V2 refuses a VNext binding before inference when preparation proof is absent', {
  skip: !isDarwin
}, async () => {
  const config = await preparedConfig({
    vnext: true,
    bindMissingPreparation: true
  });
  const store = createStore(mkdtempSync(join(tmpdir(), 'recursive-v2-missing-prep-')));
  let calls = 0;
  const result = runAdaptiveRecursiveCanaryV2(store, config, {
    runId: 'recursive-v2-missing-preparation',
    worker() { calls += 1; return null; },
    clock: () => '2026-08-04T22:50:00.000Z'
  });
  assert.equal(result.status, 'REFUSED');
  assert.equal(result.code, 'VNEXT_EXECUTION_BINDING_INVALID');
  assert.equal(calls, 0);
  assert.equal(store.exists('recursive-v2-missing-preparation'), false);
});
