...
The reserved adaptive-record lane is new-file-only:

- `src/adaptive-records.mjs`
- the five new JSON schemas
- `test/adaptive-records.test.mjs`

The lane's provider route was unavailable at freeze time. The main integrator
may implement the reserved lane and must review it under the same tests and
ownership boundary.
